...
stackS (Sta palets cap_max) palet | freeCellsS (Sta palets cap_max) == 0 = error "No hay espacio disponible"
                                  | netP palet > 10 = error "El peso del palet supera las 10 toneladas"
...
holdsS (Sta palets cap_max) (Pal ciudad) (Rou ciudades) | null palets = True 
                                                        | inOrder ciudad (destinationP (head palets)) ciudades = True -- es asi o con LAST??
                                                        | otherwise = False

popS :: Stack -> String -> Stack          -- quita del tope los paletes con destino en la ciudad indicada
popS (Sta palets cap_max) ciudad | null palets = Sta palets cap_max
                                 | destinationP (head palets) == ciudad = popS (Sta (tail palets) cap_max) ciudad -- si el destino coincide
                                | otherwise = Sta palets cap_max -- si el destino no coincide qeuda igual